...
The first corrected six-person evaluation is documented in [`results/PILOT_RESULTS.md`](../results/PILOT_RESULTS.md). With arbitrary 1-second windows, the best baseline was sEMG feature classification (67.8% balanced accuracy), while the dual-branch CNN reached 65.6%. Pressure-event-centred windows raised the IMU+sEMG CNN to 88.8% mean balanced accuracy across three seeds. The evaluated task was offline trial classification; continuous recognition and automatic product tuning were outside the experiment scope.
